...
static bool lyn_ultrakey_load_effect(struct lyn_ultrakey_filter *filter)
{
    ...
    if (!effect_path) {
        blog(LOG_ERROR,
             "[Lyn UltraKey] obs_module_file returned NULL for ultrakey.effect");
        return false;
    }
    ...
    if (!filter->effect) {
        blog(LOG_ERROR,
             "[Lyn UltraKey] effect load failed; filter will run pass-through");
        return false;
    }
...
static void *lyn_ultrakey_create(obs_data_t *settings, obs_source_t *source)
{
    ...
    blog(LOG_INFO,
         "[Lyn UltraKey] instance created (effect_loaded=%s)",
         filter->effect_loaded ? "true" : "false");
...
static void lyn_ultrakey_video_render(void *data, gs_effect_t *unused_effect)
{
    ...
    if (!filter || !filter->context || !filter->effect_loaded ||
        !filter->effect) {
    ...
    lyn_ultrakey_set_float(filter->despill_inside_param,
                           filter->despill_inside);
    lyn_ultrakey_set_float(filter->decontaminate_param,
                           filter->edge_decontaminate);
    lyn_ultrakey_set_float(filter->black_protect_param,
                           filter->black_protect);
...
static obs_properties_t *lyn_ultrakey_get_properties(void *data)
{
    ...
    obs_properties_add_color(
        props, S_KEY_COLOR, obs_module_text("LynUltraKey.KeyColor"));

    obs_properties_add_int_slider(
        props, S_SIMILARITY,
        obs_module_text("LynUltraKey.Similarity"), 0, 1000, 1);

    obs_properties_add_int_slider(
        props, S_SOFTNESS,
        obs_module_text("LynUltraKey.Softness"), 0, 1000, 1);

    obs_properties_add_int_slider(
        props, S_EDGE_CLEAN,
        obs_module_text("LynUltraKey.EdgeClean"), 0, 1000, 1);

    obs_properties_add_int_slider(
        props, S_NOISE_REJECT,
        obs_module_text("LynUltraKey.NoiseReject"), 0, 1000, 1);

    obs_properties_add_int_slider(
        props, S_HAIR_DETAIL,
        obs_module_text("LynUltraKey.HairDetail"), 0, 1000, 1);

    obs_properties_add_int_slider(
        props, S_DESPILL,
        obs_module_text("LynUltraKey.Despill"), 0, 1000, 1);

    obs_properties_add_int_slider(
        props, S_DESPILL_INSIDE,
        obs_module_text("LynUltraKey.DespillInside"), 0, 1000, 1);

    obs_properties_add_int_slider(
        props, S_DECONTAMINATE,
        obs_module_text("LynUltraKey.Decontaminate"), 0, 1000, 1);

    obs_properties_add_int_slider(
        props, S_BLACK_PROTECT,
        obs_module_text("LynUltraKey.BlackProtect"), 0, 1000, 1);
